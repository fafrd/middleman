...
export function createNotesHttpRoutes(options: { swarmManager: SwarmManager }): Hono<NodeServerEnv> {
  ...
  app.patch(
    ...
    async (c) => {
      ...
      try {
        ...
        const note = await renameNote(swarmManager.getConfig().paths.dataDir, notePath, newFilename)
        return c.json({ note })
      } catch (error) {
        return sendNotesError(error)
      }

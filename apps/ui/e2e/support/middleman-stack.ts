...
export async function startMiddlemanStack(fixture: Record<string, unknown>): Promise<StartedMiddlemanStack> {
  await Promise.all([
...
async function assertPathExists(path: string, label: string): Promise<void> {
  try {
    ...
    throw new Error(`Missing ${label} at ${path}. Run "pnpm build" before executing Playwright tests.`)
  }
}

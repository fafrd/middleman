...
test('manager can spawn a worker, switch agents, and reload persisted history', async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: `Expand manager ${managerId}`, exact: true }).click({ force: true })

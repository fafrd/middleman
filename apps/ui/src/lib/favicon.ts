...
type AgentLiveStatuses = Record<string, { status: AgentStatus; pendingCount: number }>;
...
function getAgentLiveStatus(agent: AgentDescriptor, statuses: AgentLiveStatuses): AgentStatus {
...
export function resolveWorkspaceFaviconEmoji(
  ...
  const anyStreaming = agents.some((agent) =>
    isWorkingAgentStatus(getAgentLiveStatus(agent, statuses)),
  );

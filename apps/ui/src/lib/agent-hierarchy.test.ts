...
describe("agent-hierarchy", () => {
  ...
  it("chooses fallback target preferring a primary manager", () => {
    ...
    expect(chooseFallbackAgentId(agents, ["manager-2", "manager"], "missing-agent")).toBe(
      "manager-2",
    );
  });
  ...
  it("keeps stopped and errored agents visible for restore and transcript replay", () => {
    ...
    const erroredWorker = {
      ...worker("worker-error", "manager-stopped"),
    ...
    expect(chooseFallbackAgentId([stoppedManager, erroredWorker], [], null)).toBe(
      "manager-stopped",
    );
  });
});

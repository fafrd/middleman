...
export function normalizeManagerOrder(managerOrder: string[], agents: AgentDescriptor[]): string[] {
...
export function getOrderedManagers(
  ...
  const managers = agents.filter((agent) => agent.role === "manager");
  const managerById = new Map(managers.map((manager) => [manager.agentId, manager] as const));

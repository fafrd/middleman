...
function parseSettingsAuthOAuthEventData(rawData: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error("Invalid OAuth event payload.");
  }
...
export async function updateSettingsEnvVariables(
  wsUrl: string,
...
export async function deleteSettingsEnvVariable(
  wsUrl: string,
  variableName: string,
): Promise<void> {
  const endpoint = resolveApiEndpoint(
    wsUrl,
    `/api/settings/env/${encodeURIComponent(variableName)}`,
  );
...
export async function fetchSettingsAuthProviders(wsUrl: string): Promise<SettingsAuthProvider[]> {
  ...
  const parsed = payload.providers
    .map((v) => parseSettingsAuthProvider(v))
    .filter((v): v is SettingsAuthProvider => v !== null);
  const configuredByProvider = new Map(parsed.map((entry) => [entry.provider, entry]));
  return SETTINGS_AUTH_PROVIDER_ORDER.map(
    (provider) => configuredByProvider.get(provider) ?? { provider, configured: false },
  );
}

export async function updateSettingsAuthProviders(
  wsUrl: string,
  values: Partial<Record<SettingsAuthProviderId, string>>,
): Promise<void> {
  const endpoint = resolveApiEndpoint(wsUrl, "/api/settings/auth");
  const response = await fetch(endpoint, {
    method: "PUT",
...
export async function deleteSettingsAuthProvider(
  wsUrl: string,
...
export async function startSettingsAuthOAuthLoginStream(
  ...
  const endpoint = resolveApiEndpoint(
    wsUrl,
    `/api/settings/auth/login/${encodeURIComponent(provider)}`,
  );
...
export async function submitSettingsAuthOAuthPrompt(
  wsUrl: string,
  provider: SettingsAuthProviderId,
  value: string,
): Promise<void> {

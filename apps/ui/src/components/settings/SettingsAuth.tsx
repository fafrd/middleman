      ...
      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="relative w-full flex-1">
          <Input
            ...
            placeholder={
              authStatus.configured
                ? (authStatus.maskedValue ?? metadata.placeholder)
...
export function SettingsAuth({ wsUrl }: SettingsAuthProps) {
  ...
  const [authDraftByProvider, setAuthDraftByProvider] = useState<
    Partial<Record<SettingsAuthProviderId, string>>
  >({});
  const [authRevealByProvider, setAuthRevealByProvider] = useState<
    Partial<Record<SettingsAuthProviderId, boolean>>
  >({});
  ...
  const [deletingAuthProvider, setDeletingAuthProvider] = useState<SettingsAuthProviderId | null>(
    null,
  );
  ...
  const handleSaveAuth = async (provider: SettingsAuthProviderId) => {
    ...
    if (!value) {
      setAuthError(
        `Enter a value for ${SETTINGS_AUTH_PROVIDER_META[provider].label} before saving.`,
      );
      return;
    }
    setAuthError(null);
            ...
            setOauthFlowByProvider((prev) => {
              ...
              return {
                ...
                [provider]: {
                  ...current,
                  status:
                    current.status === "waiting_for_code" ? "waiting_for_code" : "waiting_for_auth",
                  authUrl: event.url,
  ...
  return (
    <div className="flex flex-col gap-8">
      <SettingsSection label="API Keys" description="Stored in ~/.middleman/auth/auth.json">
        {authError ? (
            ...
            {SETTINGS_AUTH_PROVIDER_ORDER.map((provider) => {
              ...
              return (
                <AuthProviderRow
                  ...
                  oauthFlow={
                    oauthFlowByProvider[provider] ?? DEFAULT_SETTINGS_AUTH_OAUTH_FLOW_STATE
                  }
                  onDraftChange={(value) => {

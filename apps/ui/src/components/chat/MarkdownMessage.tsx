    ...
    <>
      ...
      >
        <ReactMarkdown
          ...
          components={{
              ...
              return (
                <p
                  className={cn(
                    ...
                    isDocument ? "mb-5 text-foreground/90 last:mb-0" : "mb-2 last:mb-0",
                  )}
                >
                  {children}
                </p>
            ...
            img({ src, alt, title }) {
              const imageSrc =
                typeof src === "string" ? (rewriteLocalFileUrl(wsUrl, src) ?? src) : null;
              if (!imageSrc) {
                return null;
              }
              ...
              return (
                <code
                  className={cn(
                    ...
                    isDocument ? "px-1.5 py-0.5 text-[13px]" : "px-1 py-0.5 text-xs",
                  )}
                >
                  {children}
                </code>
            ...
            td({ children }) {
              return (
                <td className="border border-border/50 px-3 py-2 text-foreground/90">{children}</td>
              );
            },
            strong({ children }) {
      ...
      {zoomTarget ? (
        <ContentZoomDialog
          ...
          title={
            zoomTarget.type === "mermaid" ? "Expanded Mermaid diagram" : "Expanded image preview"
          }
